...
var ComponentList=React.createClass({
	...
	render:function(){
		console.log("this is outer param&&&&&");
		...
		//用变量去接收传入到子组件里面的数据;
		var child =null;
		//判断当前的props 点击搜索以后传入数据  || 和默认state 组件初始化得到的数据;
		...
		for(var i=0;i<child.length;i++){//遍历 1.每条数据传递给子组件  2.循环叠加子组件个数
			...
			_temp.push(<ComponentListItem name={child[i]}/>);
		}
		return(
			<div style={_css.cxt}>
			    <div style={_css.list}>
	...
	clickHandle:function(event){
		...
		$.ajax({
			...
			success:function(data){
				...
				//装载新组件
				ReactDOM.render(<ComponentDetail/>,document.getElementById("list"))
			}
		})
		...
		return(
			<div style={_css.listitem}>
						<div style={_css.listimg}>
							<img onClick={this.clickHandle} title={this.props.name.goodsID} style={_css.img} src={this.props.name.goodsListImg}/>
						</div>
